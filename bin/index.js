...
import { red, green, blue, yellow } from "colorette";
...
program
  ...
  .option(
    "-b, --base [path]",
    "Sepcify path for the base language file. only take effect with -K option",
    "",
  )
  ...
  .option(
    "-K, --keyasareference",
    "Deal with the reference comment as a key",
    false,
  )
  .option(
    "-k, --keyseparator [path]",
    "Specify keyseparator you want to use, defaults to ##",
    "##",
  )
  .option("-P, --plurals <path>", "Specify path to plural forms definitions")
  .option(
    "--project <name>",
    "Specify the project-id-version when converting json to gettext",
  )
  .option("--quiet", "Silence output", false)
  .option(
    "--gettextDefaultCharset",
    "Default charset when parsing gettext files with gettext-parser",
  )
  .option(
    "--skipUntranslated",
    "Skip untranslated keys when converting into json",
    false,
  )
  .option(
    "--splitNewLine",
    "Outputs gettext multiline values as json array of strings",
    false,
  )
  ...
  .option(
    "--foldLength <value>",
    "Specify the character fold length for strings",
  )
  .option(
    "--compatibilityJSON <ver>",
    "Set to 'v4' to generate i18next@21 compatible json files",
  )
  .option(
    "--noDate",
    "Removes timestamp related lines from output file (POT-Creation-Date and PO-Revision-Date lines)",
  )
  .parse(process.argv);

const { source, target, filter, base, ...options } = program.opts();
...
function processFile(locale, source, target, options) {
  ...
  return readFile(source)
    .then((body) => {
      ...
      return mkdir(targetDir, { recursive: true }).then(() =>
        converter(locale, body, options),
      );
    })
    .then((data) => {
